...
Downstream reviewer samples such as `agent_scheduler_delay_sample.json` and
`remote_fallback_recovery_sample.json` are not generated by Forge and must not
be treated as `agent_manifest.json`, `metadata.json`, or `manifest.json` inputs.
They can later explain AIGuard evidence types such as `scheduler_delay_pattern`
and `remote_execution_recovered_by_fallback` after Runtime/Orchestrator
...
- Curated Orchestrator samples are reviewer navigation artifacts, not Forge
  build provenance, Runtime benchmark output, or deployment policy signals.
